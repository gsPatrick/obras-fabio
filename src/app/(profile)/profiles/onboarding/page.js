...
import Link from "next/link";
    ...
    const GroupMonitorStep = ({ profileId, onComplete }) => {
        ...
        const fetchGroups = useCallback(async () => {
            ...
            try {
                // A rota /groups já filtra grupos por participação do usuário e checa o plano no Backend
        ...
        // Ação para iniciar o monitoramento
        const handleMonitorGroup = async () => {
            ...
            try {
                ...
                onComplete(); // Avança para a próxima etapa (Metas/Categorias)
        ...
        // Verifica se o erro é a falta do número de WhatsApp (para exibir a mensagem correta)
        ...
        return (
            <Card className="w-full max-w-xl mx-auto">
                ...
                <CardContent className="space-y-4">
                    ...
                    {/* CRÍTICO: Exibir a mensagem de erro detalhada */}
                    {error && (
                        <div className="p-3 bg-red-100 dark:bg-red-950 border border-red-300 rounded-lg">
                            ...
                            {/* Se não for erro de número, é erro de plano ou Bot */}
                            {!isWhatsappNumberMissing && (
    ...
    const CategoriesGoalsStep = ({ profileId, onComplete }) => {
        ...
        // Lógica de CRUD de Categorias (reutilizada)
    ...
    export default function OnboardingPage() {
        ...
        // Conteúdo da Etapa 1 (Inline para simplicidade)
        const step1Content = (
            <Card className="w-full max-w-xl mx-auto">
                <CardHeader>
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-8 w-full max-w-4xl mx-auto">
                ...
                {/* Navegação de Passos (Cards-Botões) */}
                <div className="flex w-full justify-between mb-8 space-x-2">
                    {steps.map(step => (
                        ...
                        >
                            <step.icon className={cn("h-6 w-6 mx-auto mb-2", step.id <= currentStep ? "text-primary-foreground" : "text-gray-500 dark:text-gray-400")} />
                            <span className="text-sm font-semibold">{step.name}</span>
                            {step.id < currentStep && <CheckCircle className="h-4 w-4 text-primary-foreground absolute top-1 right-1" />}
                        </Card>
                    ))}
                </div>
                ...
                {currentStep > 1 && steps.find(step => step.id === currentStep)?.content}
                
                {/* Ações de Finalização (Visível apenas na última etapa) */}
                {currentStep === steps.length && (
                    <div className='mt-8 text-center'>
                        <Button onClick={handleFinalRedirect} className="text-lg">
