...
export function DataTable({ columns }) {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error("Falha ao carregar a lista de etapas.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleEdit = (data) => { setEditingData(data); setIsDialogOpen(true); };
  const handleCreate = () => { setEditingData(null); setIsDialogOpen(true); };
  const handleDeleteRequest = (step) => { setStepToDelete(step); setIsDeleteDialogOpen(true); };

  const handleConfirmDelete = async () => {
    if (!stepToDelete) return;
    try {
        await api.delete(`/steps/${stepToDelete.id}`);
        toast.success(`Etapa "${stepToDelete.name}" excluída com sucesso!`);
        fetchData();
    } catch (error) {
        toast.error(error.response?.data?.error || "Falha ao excluir a etapa.");
    } finally {
  ...
  const handleSave = async (formData) => {
    try {
        if (editingData?.id) {
            await api.put(`/steps/${editingData.id}`, formData);
            toast.success("Etapa atualizada com sucesso!");
        } else {
            await api.post('/steps', formData);
  ...
  return (
    <div>
        <div className="flex items-center justify-between py-4">
            ...
            className="max-w-sm"
            />
            <Button onClick={handleCreate}><PlusCircle className="mr-2 h-4 w-4"/>Nova Etapa</Button>
        </div>
            ...
            <Table>
                ...
                {isLoading ? (
                  <TableRow><TableCell colSpan={tableColumns.length} className="h-24 text-center">Carregando...</TableCell></TableRow>
                    ...
                    <TableRow><TableCell colSpan={tableColumns.length} className="h-24 text-center">Nenhuma etapa encontrada.</TableCell></TableRow>
                )}
                </TableBody>
            </Table>
        </div>
        ...
        <ConfirmationDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen} title="Confirmar Exclusão" description={`Tem certeza que deseja excluir a etapa "${stepToDelete?.name}"?`} onConfirm={handleConfirmDelete} confirmText="Sim, Excluir" />
    </div>
  )
}