...
export default function SolicitacoesPage() {
  ...
  React.useEffect(() => {
    ...
    // Apenas executa se o perfil permitir a seleção de empresa
    if (user && user.profile !== 'SOLICITANTE') {
        fetchContracts();
    }
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4">
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar initialFocus mode="range" selected={filters.date} onSelect={(date) => handleFilterChange('date', date)} numberOfMonths={2} />
              </PopoverContent>
            </Popover>
